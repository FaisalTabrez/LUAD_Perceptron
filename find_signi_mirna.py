...
# Apply the function to our index (the patient IDs)
df['Target'] =[get_label(idx) for idx in df.index]

# Drop any weird rows that aren't strictly 1 or 0
df = df.dropna(subset=['Target'])

# 4. The Biological Pre-Filter (Removing the "Noise")
# UCSC Xena data is usually log2 transformed. 
# We will drop any miRNA where the average expression across all patients is less than 1.0.
# If it's barely expressed, our synthetic circuit won't be able to "sense" it in real life.
miRNA_columns = df.columns.drop('Target')
mean_expression = df[miRNA_columns].mean()

# Keep only the abundant miRNAs
